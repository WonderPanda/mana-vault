...
const manaSymbolMap = new Map<string, string>(manaList.map((item) => [item.symbol, item.svg_uri]));

/**
 * Parses a mana cost string like "{5}{R}{R}" into an array of symbols
 */
...
/**
 * Renders a card's mana cost as a series of mana symbol SVGs
 *
 * @example
 * <ManaCost cost="{2}{U}{U}" />
 * <ManaCost cost="{W}{U}{B}{R}{G}" className="gap-1" symbolClassName="size-6" />
 */
export function ManaCost({ cost, className, symbolClassName }: ManaCostProps) {
  if (!cost) {
    return null;
  }
  ...
  return (
    <span className={cn("inline-flex items-center gap-0.5", className)}>
      {symbols.map((symbol, index) => {
        const svgUri = manaSymbolMap.get(symbol);
        ...
        if (!svgUri) {
          ...
          return (
            <span
              key={`${symbol}-${index}`}
              className={cn(
                "inline-flex size-4 items-center justify-center rounded-full bg-muted text-[10px] font-bold",
                symbolClassName,
              )}
            >
              {symbol.slice(1, -1)}
        ...
        return (
          <img
            ...
            src={svgUri}
            alt={symbol}
            className={cn("size-4", symbolClassName)}
          />
        );
      })}
    </span>
